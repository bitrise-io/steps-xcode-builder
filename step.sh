...
# Start the build
if [ ! -n "$CONCRETE_ACTION_EXPORT_ARCHIVE" ]; then
  export FULL_XCODEBUILD_ACTION="$CONCRETE_PROJECT_ACTION -scheme \"$CONCRETE_SCHEME\""
fi
export FULL_XCODEBUILD_ACTION="$FULL_XCODEBUILD_ACTION $XCODEBUILD_ACTION"

if [ ! -n "$CONCRETE_ACTION_EXPORT_ARCHIVE" ]; then
  export FULL_XCODEBUILD_ACTION="$FULL_XCODEBUILD_ACTION CODE_SIGN_IDENTITY=\"$CERTIFICATE_IDENTITY\" PROVISIONING_PROFILE=\"$PROFILE_UUID\" OTHER_CODE_SIGN_FLAGS=\"--keychain $CONCRETE_KEYCHAIN_PATH\""
